...
class SemanticRetriever:
    ...
    def _load(self):
        ...
        self.metadata = json.loads(
            (self.embeddings_dir / "chunk_metadata.json").read_text(encoding="utf-8")
        )
    ...
    def retrieve(self, tradition: str, probes_high: list, probes_low: list = None) -> list:
        ...
        tradition_set = set(tradition_indices)

        # Scale overfetch so that even small traditions get enough hits.
        # If tradition is X% of the index, we need ~top_k / X results globally.
        ...
        for probe_vec in probe_embeddings:
            # Search full index (filter by tradition after)
            scores, indices = self.index.search(
                probe_vec.reshape(1, -1), overfetch_k
            )

            for score, idx in zip(scores[0], indices[0]):
                if idx == -1:
                    continue
                if idx not in tradition_set:
                    continue
            ...
            # No early exit — exhaust all probes for maximum coverage
